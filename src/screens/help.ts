...
import { BUTTON_KEY, specialMoveInput, specialMovesFor } from '../game/moves.js';

const LINES: [string, string][] = [
  ['MOVE', '←  /  →   (arrow keys)'],
  ['JUMP', '↑   (hold a direction = leap)'],
  ['CROUCH', '↓   (hold to duck)'],
  ['PUNCH', 'W   fast, short range'],
  ['KICK', 'E   slower, longer, harder'],
  ...
  if (narrow) {
    if (specials.length) {
      for (const move of specials) {
        ...
        writeClipped(inner.x, `${move.shortName.padEnd(10)} ${specialMoveInput(move, fighter.facing, true)}`, THEME.text, true);
        y++;
      }
    } else {
    ...
    if (y <= contentBottom) { y++; writeClipped(inner.x, `LOW PUNCH  ↓ + ${BUTTON_KEY.punch}`); y++; }
    if (y <= contentBottom) { writeClipped(inner.x, `LOW KICK   ↓ + ${BUTTON_KEY.kick}`); }
  } else {
    ...
    if (specials.length) {
      for (const move of specials) {
        ...
        f.write(inner.x + 20, y, specialMoveInput(move, fighter.facing), THEME.text, THEME.panel);
        y++;
      }
    } else {
    ...
    if (y <= contentBottom) { writeClipped(inner.x, `LOW PUNCH`.padEnd(18), THEME.accent, true); f.write(inner.x + 20, y, `↓ + ${BUTTON_KEY.punch}`, THEME.text, THEME.panel); y++; }
    if (y <= contentBottom) { writeClipped(inner.x, `LOW KICK`.padEnd(18), THEME.accent, true); f.write(inner.x + 20, y, `↓ + ${BUTTON_KEY.kick}`, THEME.text, THEME.panel); }
  }
